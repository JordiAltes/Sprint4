...
function moviesAverageOfDirector(array, director) {
  ...
  const sumScore = moviesFrom1Director.reduce((total, movie) => {
    total += movie.score;
    return total;
  }, 0);
...
function moviesAverageByCategory(array, genre) {
  const moviesByGenre = array.filter(
    (movie) => movie.genre.includes(genre) && movie.score != ''
  );
  const sumScoreFrom1Genre = moviesByGenre.reduce((total, movie) => {
    total += movie.score;
    return total;
  }, 0);
  const lenghtOfFilmsOf1Genre = Number(moviesByGenre.length);
  const averageScoreOf1Genre = Number(
    (sumScoreFrom1Genre / lenghtOfFilmsOf1Genre).toFixed(2)
  );
...
function hoursToMinutes(array) {
  const arrayMovies = array.map((movies) => ({ ...movies }));
...
function bestFilmOfYear(array, year) {
  const newArrayOfMovies = array
    .map((movies) => ({ ...movies }))
    .filter((x) => x.year === year)
    .sort((a, b) => b.score - a.score);
  return [newArrayOfMovies[0]];
}
